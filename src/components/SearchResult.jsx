...
<div className="h-fit sticky top-0">
    ...
    <div className="hidden xl:block h-full">
        ...
        <div className="hidden xl:grid grid-cols-2 col-span-2 gap-4 w-fit sticky top-0 left-0 h-full">
            ...
            <div className="h-full w-fit">
                    ...
                    <div className="flex items-center w-full justify-center mt-5 px-3" onClick={handleClick}>
    ...
    <div className="block xl:hidden w-full">
        <div className="flex items-center justify-start w-full px-3 my-5 ">
                <Input className='h-12 w-[60%] lg:rounded-4xl items-center
                    focus-within:ring-1 focus-within:ring-green-900 focus-within:w-[80%]
                    transition-all duration-650 ease-in-out' 
                    type="input" placeholder="Keyword"
                    value={keyword}
        ...
        <div className="w-full h-full border-r">
                <h2 className="mx-5 my-2 font-bold text-sm md:text-md xl:text-lg pt-5">Price</h2>
                    <Input type="range" className="mx-5 border-none outline-none w-[80%]" min={0} max={1000} 
                    onChange={(e) => setPrice(e.target.value)} value={price}/>
                    <p className="mx-5 text-xs md:text-sm">Under CAD {price}</p>

                <Separator className="my-3"/>

                <h2 className="mx-5 mb-2 mt-3 font-bold text-sm md:text-md xl:text-lg">Condition</h2>
                <RadioGroup defaultValue={condition} onValueChange={setCondition} className="flex font-thin my-2" value={condition}>
                <div className="grid grid-cols-3 md:grid-cols-5 items-start justify-start space-x-2 ">
                        {Data.Condition.map((c, index) => (
                    <div key={index} className="flex flex-row my-2 mx-5 items-center">
                                <RadioGroupItem value={c.condition} id={`condition-${index}`} className="font-thin" />
                                <Label htmlFor={`condition-${index}`} className="mx-2 text-xs md:text-sm">{c.condition}</Label>
                    </div>
                        ))}
                </div>
                </RadioGroup>
                ...
                <h2 className="mx-5 mb-2 mt-3 font-bold text-sm md:text-md xl:text-lg">Location</h2>
                <RadioGroup defaultValue={location} onValueChange={setLocation} className="flex font-thin my-2" value={location}>
                    <div className="grid grid-cols-3 md:grid-cols-5 items-start justify-start space-x-2">
                        {Data.Location.map((loc, index) => (
                            <div key={index} className="flex flex-row my-2 mx-5 items-center">
                                <RadioGroupItem value={loc.name} id={`location-${index}`} className="font-thin" />
                                <Label htmlFor={`location-${index}`} className="mx-2 text-xs md:text-sm">{loc.name}</Label>
                            </div>
                        ))}
                    </div>
                </RadioGroup>

                <h2 className="mx-3 font-bold text-sm md:text-md xl:text-lg my-2 pt-5">Category</h2>
                    <RadioGroup defaultValue={category} onValueChange={setCategory} className="flex font-thin my-2" value={category}>
                        <div className="grid grid-cols-3 md:grid-cols-5 items-start justify-start space-x-2">
                            {Data.Category.map((category, index) => (
                                <div className="flex flex-row my-2 mx-4 items-center">
                                    <RadioGroupItem key={index} value={category.category} id={index} className="font-thin"/>
                                    <Label htmlFor={index} className="mx-2 text-xs md:text-sm mb-1">{category.category}</Label>
                                </div>
                            ))}
                        </div>
                    </RadioGroup>
                ...
                <div className="flex items-center w-full justify-center mt-5 px-3" onClick={handleClick}>
                        <div className="w-full">
                            <Button className='flex text-md rounded-md items-center justify-center gap-2 bg-emerald-800 p-2 text-white
                            hover:bg-emerald-700 transition-all w-full'><LiaSearchSolid />Search</Button>
                        </div>   
                </div>   
         </div>
    </div>
